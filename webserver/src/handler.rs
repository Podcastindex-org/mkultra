...
pub async fn context(ctx: Context) -> Response {
    ...
    match mkultra::check_session(chat_id) {
        Ok(_) => {
            ...
            //Get the comments
            let comments = get_comments_by_chat_id(chat_id).unwrap();
...
pub async fn boosts(_ctx: Context) -> Response {
    let doc = fs::read_to_string("boosts.json").expect("Something went wrong reading the file.");
    return hyper::Response::builder()
        .status(StatusCode::OK)
        .body(format!("{}", doc).into())
        .unwrap();
...
pub fn get_comments_by_chat_id(chat_id: &str) -> Result<Vec<Comment>, Box<dyn Error>> {
    let conn = Connection::open(SQLITE_FILE_COMMENTS)?;
